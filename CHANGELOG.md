...
- 图片单个最大 50 MB，音视频单个最大 1 GB，每批最多 50 个、合计 2 GB；验证格式与文件头，整批成功后才写入资料目录，失败清理本批临时附件。
- 修复大于 50 MB 的手机视频被拦截：附件以 4 MB 分块保存、上传和下载，显示进度；上传中断按已保存的位置续传，完成后流式写入电脑文件。暂存限额和过期清理防止未完成传输长期占用空间。
