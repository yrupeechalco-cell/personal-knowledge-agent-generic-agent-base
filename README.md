...
Web 工作区现已支持手机底部导航、文件抽屉和 Safari 文档导入。运行 `npm run dev:mobile`，手机连接同一网络后打开终端给出的地址，即可随代码修改实时查看界面。导入文档仅作本次只读预览，刷新后需重新导入；原生 iOS 安装、源文件写回和跨端同步尚未实现。[使用方式与验收说明](docs/MOBILE_PREVIEW.md)
