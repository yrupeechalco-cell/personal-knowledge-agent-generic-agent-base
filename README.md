...
支持图片、音频、视频导入，图片缩略图/放大、音视频播放、分类搜索与原文件导出。图片单个最大 50 MB，音视频单个最大 1 GB；以 4 MB 分块保存和传输，显示进度，上传中断后自动续传。手机导入后上传原文件，电脑已有附件打开时下载并在手机保存。识图、转写暂未接入，设备不支持的编码可导出后用其他应用打开。
