  ...
  return <section className="library-host" hidden={!visible} aria-label="文件知识库">
    ...
    {!adapter ? <div className="library-empty"><FolderSearch size={32} /><h3>请在正式安装的 App 中添加资料</h3><p>选择一个电脑文件夹，在这里浏览主题分类、关键词和知识 tip。</p><p>文件读取与编辑需要桌面 App。网页预览不会扫描你的电脑。</p></div> : <>
      ...
      {adapter.configure && <section className="library-queue" aria-label="AI 整理任务">
        ...
        <div className="library-actions">{queueActive ? <button onClick={() => void configure({ queueState: "paused" })}><Pause size={14} />暂停整理</button> : <button disabled={!modelReady || !runModel || busy || detailDirty || pending === 0} onClick={() => void configure({ queueState: "running" })}><Play size={14} />{data.queueState === "paused" ? "继续整理" : "整理全部待处理"}</button>}{failures > 0 && <button disabled={!modelReady || busy || detailDirty} onClick={() => void configure({ retryFailed: true, queueState: "running" })}>重试失败项（{failures}）</button>}</div>
