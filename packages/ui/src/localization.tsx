import { createContext, useContext, useLayoutEffect, useMemo, useState, type ReactNode } from "react";

export type AppLocale = "zh-CN" | "en";
export type TranslationValues = Record<string, string | number>;

const LOCALE_STORAGE_KEY = "knowledge-agent.locale";

const ENGLISH_MESSAGES: Record<string, string> = {
  "笔记统计": "Note statistics",
  "{count} 字": "{count} words",
  "{count} 字符": "{count} characters",
  "约 {count} 分钟阅读": "About {count} min read",
  "Markdown 编辑器": "Markdown editor",
  "斜杠命令": "Slash commands",
  "插入内容": "Insert content",
  "↑↓ 选择 · Enter 插入 · Esc 关闭": "↑↓ select · Enter insert · Esc close",
  "窗口控制": "Window controls",
  "最小化": "Minimize",
  "最大化或还原": "Maximize or restore",
  "关闭窗口": "Close window",
  "一级标题": "Heading 1",
  "二级标题": "Heading 2",
  "三级标题": "Heading 3",
  "项目符号列表": "Bullet list",
  "待办事项": "Task",
  "引用块": "Quote",
  "代码块": "Code block",
  "双向链接": "Wikilink",
  "分隔线": "Divider",
  "当前日期": "Current date",
  "当前时间": "Current time",
  "未解析：{target}": "Unresolved: {target}",
  "尚未创建的笔记": "Note not created yet",
  "点击可创建并打开这篇笔记。": "Click to create and open this note.",
  "创建唯一笔记": "Create unique note",
  "使用本地时间戳创建不会重名的笔记": "Create a non-conflicting note from the local timestamp",
  "按标题拆分当前笔记": "Split current note by heading",
  "将一个标题章节提取为新笔记，并在原位置保留双链": "Extract a heading section into a new note and leave a wikilink in its place",
  "合并笔记内容": "Merge note content",
  "把另一篇笔记复制到当前笔记，并保留来源文档": "Copy another note into the current note and keep the source document",
  "按标题拆分笔记": "Split note by heading",
  "选择要提取为新笔记的标题": "Choose a heading to extract into a new note",
  "选择要合并到当前笔记的文档": "Choose a document to merge into the current note",
  "属性数据库": "Property database",
  "{visible} / {total} 篇笔记": "{visible} / {total} notes",
  "筛选数据库": "Filter database",
  "搜索标题、路径、标签或属性": "Search titles, paths, tags, or properties",
  "排序": "Sort",
  "排序字段": "Sort field",
  "标题": "Title",
  "路径": "Path",
  "切换排序方向": "Toggle sort direction",
  "升序": "Ascending",
  "降序": "Descending",
  "暂无自定义属性": "No custom properties",
  "打开属性数据库": "Open property database",
  "按 frontmatter 属性筛选、排序和编辑全部笔记": "Filter, sort, and edit every note by frontmatter properties",
  "管理工作区布局": "Manage workspace layouts",
  "保存并恢复侧栏比例、显隐状态和当前主视图": "Save and restore sidebar proportions, visibility, and the current main view",
  "工作区布局": "Workspace layouts",
  "保存侧栏宽度、显隐状态和当前主视图": "Save sidebar widths, visibility, and the current main view",
  "布局名称": "Layout name",
  "例如：学习、项目、专注阅读": "For example: Study, Project, Focused reading",
  "保存当前布局": "Save current layout",
  "还没有保存的布局": "No saved layouts yet",
  "左栏开启": "Left sidebar on",
  "左栏关闭": "Left sidebar off",
  "Agent 开启": "Agent on",
  "Agent 收起": "Agent collapsed",
  "恢复布局": "Restore layout",
  "删除布局": "Delete layout",
  "幻灯片": "Slides",
  "演示当前笔记": "Present current note",
  "按 --- 分页，以本地幻灯片模式阅读当前笔记": "Split on --- and read the current note as a local slide deck",
  "空白演示": "Empty presentation",
  "全屏演示": "Present fullscreen",
  "关闭演示": "Close presentation",
  "当前笔记没有可演示的正文": "The current note has no presentable content",
  "脚注": "Footnotes",
  "返回脚注引用": "Return to footnote reference",
  "打开格式转换器": "Open format converter",
  "批量转换 Roam、Bear 与旧属性 Markdown 格式": "Batch-convert Roam, Bear, and deprecated property Markdown formats",
  "格式转换器": "Format converter",
  "将其他应用的 Markdown 语法转换为当前格式": "Convert Markdown syntax from other apps into the current format",
  "将 #tag 与 #[[tag]] 转为 [[tag]]": "Convert #tag and #[[tag]] to [[tag]]",
  "将 ^^highlight^^ 转为 ==highlight==": "Convert ^^highlight^^ to ==highlight==",
  "将 {{[[TODO]]}} 转为 [ ]": "Convert {{[[TODO]]}} to [ ]",
  "将 ::highlight:: 转为 ==highlight==": "Convert ::highlight:: to ==highlight==",
  "将 alias、tag、cssclass 转为当前列表属性": "Convert alias, tag, and cssclass to current list properties",
  "旧属性格式": "Deprecated property formats",
  "将修改 {notes} 篇笔记，共 {changes} 处": "{notes} notes and {changes} occurrences will be changed",
  "这是批量操作。确认后改动会进入现有草稿与自动保存流程。": "This is a batch operation. After confirmation, changes enter the existing draft and autosave flow.",
  "开始转换": "Start conversion",
  "切换侧栏": "Toggle sidebar",
  "切换到深色主题": "Switch to dark theme",
  "切换到浅色主题": "Switch to light theme",
  "存储空间": "Storage",
  "搜索": "Search",
  "笔记": "Notes",
  "只读来源不能新建笔记": "New notes are disabled for read-only sources",
  "新建笔记": "New note",
  "保存中": "Saving",
  "改动": "Changes",
  "回收站": "Trash",
  "未连接": "Disconnected",
  "未连接知识库": "No knowledge base connected",
  "本地知识库": "Local knowledge base",
  "本地 vault": "Local vault",
  "GitHub 公开库 · 只读": "Public GitHub repository · Read only",
  "工具栏": "Toolbar",
  "命令面板": "Command palette",
  "快速切换": "Quick switcher",
  "全库搜索": "Search all notes",
  "输入命令": "Type a command",
  "输入标题、路径或别名": "Type a title, path, or alias",
  "搜索正文，支持 file:、path:、tag:、task: 和 [属性]": "Search content with file:, path:, tag:, task:, and [property]",
  "结果": "Results",
  "输入关键词开始全库搜索": "Type a query to search all notes",
  "没有匹配结果": "No matching results",
  "按标题、路径或别名打开笔记": "Open notes by title, path, or alias",
  "搜索正文、标签、任务和属性": "Search content, tags, tasks, and properties",
  "创建一篇新的 Markdown 笔记": "Create a new Markdown note",
  "创建新笔记": "Create new note",
  "打开今日笔记": "Open today's note",
  "打开或创建今天的日记": "Open or create today's daily note",
  "插入模板": "Insert template",
  "选择要插入的模板": "Choose a template to insert",
  "从当前知识库的模板文件夹插入内容": "Insert content from the current knowledge base's template folder",
  "配置模板与日记": "Configure templates and daily notes",
  "设置模板文件夹、日记目录、日期格式和日记模板": "Set the template folder, daily-note folder, date format, and daily template",
  "模板与日记设置": "Template and daily-note settings",
  "模板与日记": "Templates and daily notes",
  "这些设置只保存在当前设备，并按知识库分别记录。": "These settings stay on this device and are stored separately for each knowledge base.",
  "模板文件夹": "Template folder",
  "日记文件夹": "Daily-note folder",
  "日期格式": "Date format",
  "支持 YYYY、YY、MM、M、DD、D。文件名非法字符会转换为连字符。": "Supports YYYY, YY, MM, M, DD, and D. Invalid filename characters become hyphens.",
  "日记模板": "Daily-note template",
  "不使用模板": "No template",
  "模板变量：{{title}}、{{date}}、{{time}}": "Template variables: {{title}}, {{date}}, {{time}}",
  "保存": "Save",
  "打开随机笔记": "Open random note",
  "从当前知识库随机打开一篇笔记": "Open a random note from the current knowledge base",
  "打开关系图谱": "Open relationship graph",
  "查看文件关系和知识地形": "View file relationships and knowledge terrain",
  "打开知识画布": "Open knowledge canvas",
  "在自由画布中连接文本、笔记和分组": "Connect text, notes, and groups on a freeform canvas",
  "打开资源查询": "Open resource explorer",
  "浏览当前知识库或只读硬盘结构": "Browse the current knowledge base or a read-only disk structure",
  "打开回收站": "Open Trash",
  "预览、恢复仍在保留期内的删除内容": "Preview and restore deleted content within its retention period",
  "切换编辑/阅读模式": "Toggle edit/reading mode",
  "切换当前笔记的编辑与预览状态": "Toggle the current note between edit and preview",
  "切换左侧栏": "Toggle left sidebar",
  "显示或隐藏文件树和标签": "Show or hide the file tree and tags",
  "笔记导航": "Note navigation",
  "属性": "Properties",
  "大纲": "Outline",
  "链接": "Links",
  "书签": "Bookmarks",
  "收藏当前笔记": "Bookmark current note",
  "取消当前笔记书签": "Remove bookmark from current note",
  "暂无属性": "No properties",
  "新属性名称": "New property name",
  "新属性值": "New property value",
  "属性名": "Property",
  "值": "Value",
  "添加属性": "Add property",
  "当前笔记没有标题大纲": "This note has no heading outline",
  "出链": "Outgoing links",
  "反链": "Backlinks",
  "没有出链": "No outgoing links",
  "没有反链": "No backlinks",
  "暂无书签": "No bookmarks",
  "切换智能体栏": "Toggle Agent sidebar",
  "显示或收起 Note Agent": "Show or collapse Note Agent",
  "打开存储空间": "Open storage",
  "打开、创建知识库或读取硬盘结构": "Open or create a knowledge base, or inspect disk structure",
  "打开应用设置": "Open app settings",
  "查看版本、更新、安全和语言设置": "View version, update, security, and language settings",
  "应用设置": "App settings",
  "关闭设置": "Close settings",
  "设置分类": "Settings categories",
  "个人知识库 Agent": "Personal Knowledge Agent",
  "常规": "General",
  "外观与语言": "Appearance & language",
  "笔记与文件": "Notes & files",
  "图谱与画布": "Graph & canvas",
  "快捷键": "Hotkeys",
  "更新与安全": "Updates & security",
  "管理当前知识库、工作区布局和常用入口。": "Manage the current knowledge base, workspace layouts, and common entry points.",
  "当前知识库": "Current knowledge base",
  "管理存储空间": "Manage storage",
  "管理布局": "Manage layouts",
  "查看并恢复三十天内删除的本地文档。": "Review and restore local documents deleted within the last 30 days.",
  "统一控制 App 和网页端的视觉主题与界面语言。": "Control the visual theme and interface language shared by the App and Web.",
  "界面主题": "Interface theme",
  "主题选择会保存在当前设备。": "The selected theme is saved on this device.",
  "浅色": "Light",
  "深色": "Dark",
  "界面语言": "Interface language",
  "一个界面内切换中文或英文。": "Switch between Chinese and English in the same interface.",
  "配置模板、日记、格式转换和文件工作流。": "Configure templates, daily notes, format conversion, and file workflows.",
  "打开设置": "Open settings",
  "格式转换": "Format conversion",
  "批量转换外部 Markdown 与旧属性格式。": "Batch-convert external Markdown and legacy property formats.",
  "打开转换器": "Open converter",
  "本地存储": "Local storage",
  "控制文件关系图谱的筛选、文字和空间尺度。": "Control filtering, labels, and spatial scale for the file relationship graph.",
  "图谱筛选": "Graph filter",
  "按节点标题或路径筛选文件关系图谱。": "Filter the file relationship graph by node title or path.",
  "缩小时仍可关闭文字，减少高密度遮挡。": "Hide labels when needed to reduce overlap in dense views.",
  "包括当前没有关系线的孤立文档。": "Include isolated documents that currently have no relationship edges.",
  "调节关系图谱节点之间的整体距离。": "Adjust the overall distance between relationship graph nodes.",
  "统一配置模型、笔记 Agent 模式和本机 API 连接。": "Configure the model, note Agent mode, and local API connection in one place.",
  "选择当前 Agent 调用的模型。": "Choose the model used by the current Agent.",
  "智能体模式": "Agent mode",
  "轮换密钥": "Rotate key",
  "检查": "Validate",
  "按键完成导航、编辑、视图切换和 Agent 操作。点击快捷键后直接按下新的组合键。": "Navigate, edit, switch views, and operate the Agent from the keyboard. Select a hotkey and press a new combination.",
  "全部恢复默认": "Reset all",
  "筛选快捷键": "Filter hotkeys",
  "搜索命令或快捷键": "Search commands or hotkeys",
  "请按新的快捷键…": "Press a new hotkey…",
  "未设置": "Not assigned",
  "清除快捷键": "Clear hotkey",
  "恢复默认快捷键": "Restore default hotkey",
  "快捷键必须包含 Ctrl、Alt、Meta，或使用 F1-F12。": "A hotkey must include Ctrl, Alt, or Meta, or use F1-F12.",
  "已用于": "is already assigned to",
  "导航": "Navigation",
  "视图": "Views",
  "工作区": "Workspace",
  "更新": "Updates",
  "查看版本、发布者、签名策略、更新说明和安全规则。": "View the version, publisher, signature policy, release notes, and safety rules.",
  "软件更新": "Software updates",
  "打开桌面端更新管理，检查版本与更新说明。": "Open desktop update management to check the version and release notes.",
  "检查更新": "Check for updates",
  "回收站保留": "Trash retention",
  "本地删除内容保留三十天，之后按真实系统时间清理。": "Locally deleted content is retained for 30 days, then cleaned using real system time.",
  "查看回收站": "View Trash",
  "API key 仅保存在本机安全存储中；知识库正文不会因为设置操作被上传。": "The API key stays in secure local storage; knowledge-base content is never uploaded by changing settings.",
  "打开命令面板": "Open command palette",
  "搜索并执行 App 中的全部命令": "Search and run every App command",
  "后退到上一个标签页": "Go back to the previous tab",
  "在当前打开的工作区标签中向后移动": "Move backward through open workspace tabs",
  "前进到下一个标签页": "Go forward to the next tab",
  "在当前打开的工作区标签中向前移动": "Move forward through open workspace tabs",
  "切换到左侧标签页": "Switch to the tab on the left",
  "不使用鼠标切换到左边的标签页": "Switch to the tab on the left without using the mouse",
  "切换到右侧标签页": "Switch to the tab on the right",
  "不使用鼠标切换到右边的标签页": "Switch to the tab on the right without using the mouse",
  "关闭当前标签页": "Close current tab",
  "关闭当前工作区标签并返回相邻标签": "Close the current workspace tab and return to an adjacent tab",
  "聚焦笔记编辑器": "Focus note editor",
  "把键盘焦点移到当前笔记正文": "Move keyboard focus to the current note body",
  "聚焦智能体输入": "Focus Agent input",
  "展开智能体并把键盘焦点移到输入框": "Expand the Agent and move keyboard focus to its prompt",
  "新建子智能体会话": "New sub-agent session",
  "创建一个与现有记忆互不干扰的智能体会话": "Create an Agent session with memory independent from existing sessions",
  "刷新当前智能体会话": "Reset current Agent session",
  "清空当前会话并保留可回溯历史": "Clear the current session while keeping recoverable history",
  "上传智能体上下文": "Upload Agent context",
  "选择图片或文档并加入当前智能体上下文": "Choose images or documents to add to the current Agent context",
  "切换浅色/深色主题": "Toggle light/dark theme",
  "立即切换整个工作台的视觉主题": "Immediately switch the visual theme for the whole workspace",
  "切换界面语言": "Switch interface language",
  "在中文与英文界面之间切换": "Switch between the Chinese and English interfaces",
  "打开快捷键设置": "Open hotkey settings",
  "查看、修改、清除或恢复全部快捷键": "View, change, clear, or restore all hotkeys",
  "集中管理外观、文件、图谱、智能体、快捷键和更新": "Manage appearance, files, graphs, the Agent, hotkeys, and updates in one place",
  "关系图谱": "Relationship graph",
  "资源查询": "Resource explorer",
  "设置": "Settings",
  "打开本地知识库": "Open local knowledge base",
  "新建会话笔记": "New note tab",
  "磁盘结构": "Disk structure",
  "公开知识库": "Public knowledge base",
  "读书": "Library",
  "筛选笔记": "Filter notes",
  "标签": "Tags",
  "标签分类": "Tag classification",
  "筛选此标签": "Filter by this tag",
  "清除标签筛选": "Clear tag filter",
  "清除": "Clear",
  "暂无标签": "No tags",
  "安全状态": "Safety",
  "只读": "Read only",
  "已排除": "Excluded",
  "仅枚举名称、路径和层级；正文读取与写入均已关闭。": "Only names, paths, and hierarchy are listed. Content reading and all writes are disabled.",
  "调整左侧栏宽度": "Resize left sidebar",
  "调整右侧 Agent 栏宽度": "Resize Agent sidebar",
  "后退": "Back",
  "前进": "Forward",
  "开始": "Start",
  "知识作用图谱": "Knowledge impact map",
  "文件关系图谱": "File relationship graph",
  "只读文件浏览": "Read-only file browser",
  "图谱观察方式": "Graph perspective",
  "知识地形": "Knowledge terrain",
  "标签体系": "Tag system",
  "标签知识图谱": "Tag knowledge map",
  "知识观察域": "Knowledge view domain",
  "分类域": "Classification",
  "脉络域": "Connections",
  "应用域": "Applications",
  "来源域": "Sources",
  "知识分类": "Knowledge classification",
  "知识脉络": "Knowledge connections",
  "应用视角": "Application view",
  "来源分布": "Source distribution",
  "概念": "Concept",
  "对象": "Entity",
  "分布": "Distribution",
  "连接": "Connections",
  "应用": "Application",
  "关闭标签详情": "Close tag details",
  "标签知识图谱尚未形成": "The tag knowledge map has not formed yet",
  "先为资料添加标签，或在笔记中使用 Agent 拆解。": "Add tags to a document or extract them with the Agent from a note.",
  "文件关系": "File relationships",
  "正在构建知识地形": "Building knowledge terrain",
  "正在构建标签知识图谱": "Building tag knowledge map",
  "图谱设置": "Graph settings",
  "筛选": "Filter",
  "筛选图谱节点": "Filter graph nodes",
  "节点标题或路径": "Node title or path",
  "显示": "Display",
  "显示标签": "Show labels",
  "显示孤立节点": "Show isolated nodes",
  "力": "Physics",
  "关系半径": "Relationship radius",
  "图谱关系半径": "Graph relationship radius",
  "关闭菜单": "Close menu",
  "在本文档中新建": "Create inside this document",
  "重命名": "Rename",
  "复制": "Copy",
  "剪切": "Cut",
  "粘贴到这里": "Paste here",
  "删除": "Delete",
  "确认删除图谱文档": "Confirm graph document deletion",
  "关闭": "Close",
  "确认移除这些文档？": "Remove these documents?",
  "确认移除这个文档？": "Remove this document?",
  "这一步只会加入回收站待写回；写回本地后会移入 .knowledge-agent-trash，并保留 30 天。": "This only stages the documents for Trash. After local write-back they move to .knowledge-agent-trash and are retained for 30 days.",
  "取消": "Cancel",
  "加入回收站": "Move to Trash",
  "配置 DeepSeek API key": "Configure DeepSeek API key",
  "首次使用在线 Agent": "First use of the online Agent",
  "填写你的 DeepSeek API key": "Enter your DeepSeek API key",
  "API key 由 Windows DPAPI 按当前用户加密保存在本机，不会写入知识库或 GitHub。": "The API key is encrypted for the current Windows user with DPAPI. It is never written to the knowledge base or GitHub.",
  "稍后": "Later",
  "保存并启用": "Save and enable",
  "本地存储空间": "Local storage",
  "关闭存储面板": "Close storage panel",
  "本机磁盘": "Local drives",
  "此电脑": "This PC",
  "打开已有文件夹": "Open existing folder",
  "选择其他位置只读浏览": "Browse another location read only",
  "GitHub 公开知识库": "Public GitHub knowledge base",
  "只读 · 无需登录": "Read only · No sign-in required",
  "GitHub 公开仓库地址": "Public GitHub repository address",
  "owner/repo 或 GitHub 仓库网址": "owner/repo or GitHub repository URL",
  "打开": "Open",
  "打开官方示例知识库": "Open official example knowledge base",
  "读取公开仓库中的安全 Markdown 并建立文件树与图谱；不会提交、删除或改写仓库内容。": "Read safe Markdown from a public repository and build its file tree and graph. The repository is never committed to, deleted from, or rewritten.",
  "新建文件夹名称": "New folder name",
  "选择位置并新建": "Choose location and create",
  "只读扫描不会读取文件正文、不会创建草稿、不会写入或删除所选目录中的任何文件。": "Read-only scanning does not read file content, create drafts, write files, or delete anything in the selected directory.",
  "浏览器入口只能打开授权文件夹；只读磁盘结构扫描和新建任意本机文件夹请使用桌面 App。": "The browser can only open folders you authorize. Use the desktop App to scan disk structure or create folders anywhere.",
  "关闭改动面板": "Close changes panel",
  "当前没有草稿改动。": "There are no draft changes.",
  "正在写回...": "Writing back...",
  "写回本地": "Write to local files",
  "仅桌面端可写回": "Write-back is desktop only",
  "改动与回收站": "Changes and Trash",
  "关闭改动与回收站面板": "Close changes and Trash panel",
  "当前没有待写回改动。": "There are no pending changes.",
  ".knowledge-agent-trash · 保留 30 天，到期自动清理": ".knowledge-agent-trash · Retained for 30 days, then removed automatically",
  "回收站为空。": "Trash is empty.",
  "已阻止": "Blocked",
  "恢复中": "Restoring",
  "恢复": "Restore",
  "Agent 审核": "Agent review",
  "连接你的知识库": "Connect your knowledge base",
  "打开一个存放 Markdown 文档的普通文件夹。新安装的应用和网站不会预置、复制或上传任何笔记。": "Open any regular folder containing Markdown documents. A new App installation or website session never bundles, copies, or uploads notes.",
  "新建知识库文件夹": "Create knowledge base folder",
  "只有你主动选择的文件夹会被读取。": "Only folders you explicitly choose are read.",
  "当前浏览器不支持文件夹选择器，请使用最新版 Chrome 或 Edge。": "This browser does not support folder selection. Use the latest Chrome or Edge.",
  "当前磁盘目录": "Current disk directory",
  "只读文件浏览器": "Read-only file browser",
  "上一级": "Parent folder",
  "刷新当前目录": "Refresh current folder",
  "当前文件路径": "Current file path",
  "搜索当前目录": "Search current folder",
  "文件和文件夹": "Files and folders",
  "名称": "Name",
  "类型": "Type",
  "大小": "Size",
  "修改时间": "Modified",
  "当前目录没有匹配项。": "No matching items in this folder.",
  "当前目录超过 1000 项，仅显示前 1000 项。进入子文件夹可继续浏览。": "This folder has more than 1,000 items. Only the first 1,000 are shown; open a subfolder to continue.",
  "只读文件预览": "Read-only file preview",
  "只读预览": "Read-only preview",
  "关闭预览": "Close preview",
  "位置": "Location",
  "修改": "Modified",
  "（文档没有可显示的文字内容）": "(This document has no displayable text content.)",
  "本视图没有写入、重命名或删除权限": "This view has no write, rename, or delete permission",
  "文件夹": "Folder",
  "符号链接": "Symbolic link",
  "其他": "Other",
  "文件": "File",
  "搜索标题、路径、标签或正文": "Search titles, paths, tags, or content",
  "全部文档": "All documents",
  "文档列表": "Document list",
  "没有匹配的文档。": "No matching documents.",
  "选择一篇笔记开始。": "Select a note to begin.",
  "编辑": "Edit",
  "阅读": "Read",
  "资料词云": "Document tag cloud",
  "文章资料卡": "Document profile",
  "编辑资料卡": "Edit document profile",
  "保存资料卡": "Save document profile",
  "取消编辑资料卡": "Cancel profile editing",
  "一句话简介": "One-line summary",
  "用一句话说明这篇资料解决了什么问题或提供了什么价值": "Summarize the problem this document solves or the value it provides",
  "文档类型": "Document type",
  "论文、文章、书籍、摘录、笔记…": "Paper, article, book, excerpt, note…",
  "资料分类": "Classification",
  "领域 / 主题 / 项目": "Domain / topic / project",
  "来源": "Source",
  "网址、书名、机构或采集渠道": "URL, book, organization, or collection channel",
  "作者": "Author",
  "作者、机构或整理者": "Author, organization, or curator",
  "录入日期": "Entry date",
  "发布日期": "Publication date",
  "未填写": "Not set",
  "尚未填写一句话简介。": "No one-line summary yet.",
  "关键词": "Keywords",
  "在下方标签体系中添加关键词": "Add keywords in the tag system below",
  "标签类型筛选": "Tag type filter",
  "全部": "All",
  "主题概念": "Topics",
  "当前类型下没有标签。": "No tags in this type.",
  "颗粒度": "Granularity",
  "概览": "Overview",
  "主题": "Topics",
  "标准": "Standard",
  "细分": "Detailed",
  "术语": "Terminology",
  "标签拆解颗粒度": "Tag extraction granularity",
  "使用 Agent 拆解标签": "Extract tags with Agent",
  "编辑标签": "Edit tags",
  "完成标签编辑": "Finish editing tags",
  "重命名标签": "Rename tag",
  "确认重命名": "Confirm rename",
  "删除标签": "Remove tag",
  "新标签": "New tag",
  "添加标签": "Add tag",
  "尚无标签，使用 Agent 拆解或手动添加。": "No tags yet. Extract them with the Agent or add one manually.",
  "当前笔记关系小图": "Current note mini graph",
  "拖拽图谱节点到这里删除": "Drag graph nodes here to delete",
  "添加当前笔记到 Agent 上下文": "Add current note to Agent context",
  "添加文件或图片到 Agent 上下文": "Add files or images to Agent context",
  "添加 Agent 附件": "Add Agent attachments",
  "Agent 附件": "Agent attachments",
  "移除附件": "Remove attachment",
  "从 Agent 上下文移除，不会删除本地文件": "Remove from Agent context without deleting the local file",
  "图片 OCR": "Image OCR",
  "文档文本": "Document text",
  "文本文件": "Text file",
  "已截断": "Truncated",
  "模型连接状态": "Model connection status",
  "Agent 设置": "Agent settings",
  "Agent 设置：模型、推理强度和权限": "Agent settings: model, reasoning effort, and permissions",
  "需要填写本机 DeepSeek API key 后才能使用在线模型。": "Enter a local DeepSeek API key to use an online model.",
  "只读磁盘结构模式：Agent 已暂停，不会读取正文、创建草稿或执行本地操作。": "Read-only disk structure mode: the Agent is paused and cannot read content, create drafts, or perform local actions.",
  "子 Agent 会话": "Sub-agent sessions",
  "切换到子 Agent": "Switch to sub-agent",
  "Agent 工具": "Agent tools",
  "新建子智能体": "New sub-agent",
  "新建子智能体：开启一个独立聊天": "New sub-agent: start an independent conversation",
  "刷新当前聊天": "Reset current conversation",
  "刷新当前聊天：清空当前 Agent 会话": "Reset current conversation: clear this Agent session",
  "回溯最近聊天": "Restore recent conversation",
  "回溯最近聊天：恢复上一段 Agent 会话和记忆": "Restore the previous Agent session and its memory",
  "只读磁盘结构模式已暂停 Agent": "The Agent is paused in read-only disk structure mode",
  "模型状态": "Model status",
  "关闭 Agent 设置": "Close Agent settings",
  "切换模型、笔记 Agent 模式和本机模型连接。": "Switch the model, note Agent mode, and local model connection.",
  "模型": "Model",
  "模型连接": "Model connection",
  "Codex 已连接": "Codex connected",
  "Codex 未连接": "Codex not connected",
  "本机密钥已保存": "Local key saved",
  "未配置密钥": "No key configured",
  "配置": "Configure",
  "笔记智能体": "Note Agent",
  "知识库工作台智能控制器": "Knowledge workspace controller",
  "已连接": "Connected",
  "离线": "Offline",
  "确认应用": "Confirm apply",
  "应用改动": "Apply",
  "智能体输入": "Agent prompt",
  "可以总结当前笔记、建议链接、生成目录或整理内容": "Try summarizing the current note, suggesting links, generating an index, or organizing content",
  "运行中": "Running",
  "发送": "Send",
  "推理强度": "Effort",
  "低": "Low",
  "中": "Medium",
  "高": "High",
  "上下文用量暂不可用": "Context usage unavailable",
  "本地": "Local",
  "本地离线模式": "Local offline mode",
  "智能体": "Agent",
  "收起智能体": "Collapse Agent",
  "Windows DPAPI 已加密": "Encrypted with Windows DPAPI",
  "由系统环境变量提供": "Provided by a system environment variable",
  "密钥已配置": "Key configured",
  "最近轮换：{time}": "Last rotated: {time}",
  "最近验证：{time}": "Last validated: {time}",
  "有效性检查通过": "Credential validation passed",
  "密钥已失效": "Credential is invalid",
  "删除本机保存的 DeepSeek API key？删除后在线 Agent 将停止工作。": "Delete the locally stored DeepSeek API key? Online Agent features will stop working.",
  "尚未检查有效性": "Not validated yet",
  "轮换": "Rotate",
  "验证": "Validate",
  "没有可见行变化。": "No visible line changes.",
  "本地工具": "Local tools",
  "知识地形尚未形成": "Knowledge terrain has not formed yet",
  "当前知识库没有可分析的 Markdown 文档。": "This knowledge base has no Markdown documents to analyze.",
  "宏观知识地形": "Macro knowledge terrain",
  "知识领域类型": "Knowledge domain types",
  "项目": "Project",
  "专题": "Topic",
  "方法": "Method",
  "资料": "Reference",
  "返回宏观知识地形": "Back to macro knowledge terrain",
  "领域影响": "Domain impact",
  "原文双链": "Explicit wiki link",
  "待确认关联": "Suggested relationship",
  "结构": "Structure",
  "证据": "Evidence",
  "独特": "Uniqueness",
  "问题": "Question",
  "决策": "Decision",
  "成果": "Output",
  "背景": "Background",
  "知识专题": "Knowledge topic",
  "方法体系": "Method system",
  "资料集合": "Reference collection",
  "复杂整理、长上下文分析、重要写作提案。": "Complex organization, long-context analysis, and substantial writing proposals.",
  "快速总结、轻量问答、低延迟浏览辅助。": "Fast summaries, lightweight Q&A, and low-latency browsing assistance.",
  "日常笔记 Agent": "Daily note Agent",
  "围绕当前笔记做总结、问答、轻量改写和安全提案。": "Summarize, answer questions, lightly rewrite, and propose safe changes around the current note.",
  "整理归档 Agent": "Organization Agent",
  "更关注标题、层级、MOC、目录和跨文件整理建议。": "Focus on titles, hierarchy, MOCs, indexes, and cross-file organization.",
  "建链图谱 Agent": "Link graph Agent",
  "更关注双链、反链、未解析概念和语义关系候选。": "Focus on links, backlinks, unresolved concepts, and semantic relationship candidates.",
  "仅使用本地笔记工具，不调用在线模型。": "Use local note tools only; do not call an online model.",
  "尚未连接知识库。请选择一个 Markdown 文件夹开始使用。": "No knowledge base is connected. Choose a Markdown folder to begin.",
  "知识库已载入。": "Knowledge base loaded.",
  "未能载入知识库。": "Could not load the knowledge base.",
  "当前入口不支持读取 GitHub 公开仓库。": "This application entry does not support public GitHub repositories.",
  "请输入 GitHub 公开仓库地址或 owner/repo。": "Enter a public GitHub repository URL or owner/repo.",
  "正在从 GitHub 读取公开知识库的文件树和 Markdown 正文...": "Reading the public repository file tree and Markdown content from GitHub...",
  "GitHub 公开库读取失败。": "Could not read the public GitHub repository.",
  "当前入口不支持只读磁盘结构扫描；请使用桌面 App。": "This entry does not support read-only disk scanning. Use the desktop App.",
  "当前入口不支持新建本地文件夹；请在桌面 App 中使用。": "This entry cannot create local folders. Use the desktop App.",
  "当前来源为只读模式。请打开本地知识库后再新建或修改文档。": "This source is read only. Open a local knowledge base before creating or editing documents.",
  "当前来源为只读模式，不能删除文档。": "Documents cannot be deleted from a read-only source.",
  "当前来源为只读模式，不支持删除。原始文件未被修改。": "Deletion is disabled for this read-only source. Original files were not changed.",
  "当前来源为只读模式，不能写回文件。": "Files cannot be written back to a read-only source.",
  "当前没有需要写回的草稿。": "There are no drafts to write back.",
  "正在思考…你可以继续浏览、编辑或操作图谱。": "Thinking... You can continue browsing, editing, or using the graph.",
  "左键进入，右键删除": "Left-click to open; right-click to delete",
  "待创建": "Not created yet",
  "文档": "Document",
  "将单个图谱节点拖到这里删除": "Drag a single graph node here to delete it",
  "知识根系": "Knowledge root map",
  "关系置信度": "Relationship confidence",
  "相对贡献": "Relative contribution",
  "加入 Agent 上下文": "Add to Agent context",
  "当前没有可上传的笔记": "No note is available to add",
  "{path} 被安全规则阻止：{reason}": "{path} was blocked by a safety rule: {reason}",
  "删除时间：{deleted} · 到期时间：{expires}": "Deleted: {deleted} · Expires: {expires}",
  "请先处理待写回改动，再恢复回收站文档": "Resolve pending write-back changes before restoring this document",
  "Agent 已授权，恢复到原路径": "Authorized by the Agent; restore to the original path",
  "交给 Agent 审核回溯权限": "Ask the Agent to review restore permission",
  "暂时无法检查更新": "Unable to check for updates right now",
  "更新安装失败，请稍后再试": "The update could not be installed. Please try again later.",
  "更新安装失败": "Update installation failed",
  "更新检查失败": "Update check failed",
  "重试安装": "Retry installation",
  "更新检查未完成": "Update check did not complete",
  "稍后提醒": "Remind me later",
  "包含最新功能、体验改进与问题修复。": "Includes the latest features, experience improvements, and fixes.",
  "正在下载安装包…": "Downloading the update...",
  "更新下载进度": "Update download progress",
  "更新将在应用重新启动后完整生效。": "The update will take full effect after the App restarts.",
  "立即更新": "Update now",
  "重新检查": "Check again",
  "知道了": "Got it",
  "更新与信任": "Updates and trust",
  "知识库智能体更新": "Knowledge Agent Update",
  "控制启动时自动检查，并核对当前版本、发布者与安装前签名验证方式。": "Control automatic checks and verify the current version, publisher, and pre-install signature policy.",
  "当前版本": "Current version",
  "目标版本": "Target version",
  "发布者": "Publisher",
  "签名验证": "Signature verification",
  "已通过 Tauri Ed25519 验证": "Verified by Tauri Ed25519",
  "安装前强制执行 Tauri Ed25519 验证": "Tauri Ed25519 verification is required before installation",
  "启动时及每 6 小时自动检查更新": "Check for updates at startup and every 6 hours",
  "立即检查": "Check now",
  "图谱镜头工具": "Graph camera tools",
  "适配全部节点": "Fit all nodes",
  "重置镜头": "Reset camera",
  "按顶层文件夹聚类": "Cluster by top-level folder",
  "按知识类型聚类": "Cluster by knowledge type",
  "显示小地图": "Show minimap",
  "隐藏小地图": "Hide minimap",
  "图谱小地图": "Graph minimap",
  "可恢复删除": "Recoverable deletion",
  "删除的 Markdown 文档保存在当前知识库的 .knowledge-agent-trash/files 中，精确保留 30 天。": "Deleted Markdown documents are stored under .knowledge-agent-trash/files in the current knowledge base and retained for exactly 30 days.",
  "个可恢复文档": "recoverable documents",
  "恢复不会覆盖同名文档；若原路径已被占用，会生成唯一恢复名称。每次恢复前均可查看内容、原路径和到期时间。": "Restore never overwrites a same-name document. If the original path is occupied, a unique restore name is created. Content, original path, and expiry are available before every restore.",
  "回收站为空": "Trash is empty",
  "删除后的文档会在这里保留 30 天。": "Deleted documents are retained here for 30 days.",
  "到期：{time}": "Expires: {time}",
  "读取中…": "Loading...",
  "恢复中…": "Restoring...",
  "恢复预览": "Restore preview",
  "物理位置": "Physical location",
  "删除时间": "Deleted",
  "自动清理": "Automatic purge",
  "剩余时间": "Time remaining",
  "选择文档查看恢复预览": "Select a document to preview its restore",
  "这里只读取回收站副本，不会修改原始知识库。": "This view only reads the Trash copy and does not modify the original knowledge base.",
  "文档较大，预览仅显示前 1 MB；恢复时仍会恢复完整原文件。": "This document is large. The preview shows only the first 1 MB; restore still returns the complete original file.",
  "未知": "Unknown",
  "根目录": "Root",
  "知识画布": "Knowledge canvas",
  "文本": "Text",
  "表格": "Table",
  "新文本": "New text",
  "新表格": "New table",
  "列 1": "Column 1",
  "列 2": "Column 2",
  "列 3": "Column 3",
  "列": "Column",
  "行": "Row",
  "添加文本卡片": "Add text card",
  "添加表格卡片": "Add table card",
  "添加知识库笔记": "Add knowledge base note",
  "将所选卡片组成主题分组": "Group selected cards",
  "主题分组": "Topic group",
  "分组名称": "Group name",
  "卡片标题": "Card title",
  "从此卡片建立连线": "Connect from this card",
  "撤销": "Undo",
  "重做": "Redo",
  "适应全部内容": "Fit all content",
  "删除所选内容": "Delete selected content",
  "正在保存": "Saving",
  "保存失败": "Save failed",
  "只读画布": "Read-only canvas",
  "已保存": "Saved",
  "画布": "Canvas",
  "搜索笔记": "Search notes",
  "没有匹配的笔记": "No matching notes",
  "从卡片开始组织知识": "Start organizing with cards",
  "添加文本、表格或真实笔记，再用有意义的连线表达关系。": "Add text, tables, or real notes, then connect them with meaningful relationships.",
  "写下观点、问题或结论…": "Write an idea, question, or conclusion...",
  "原笔记已不存在": "The source note no longer exists",
  "调整卡片大小": "Resize card",
  "连线含义": "Connection meaning",
  "关系": "Relationship",
  "自定义说明": "Custom label",
  "相关": "Related",
  "支持": "Supports",
  "冲突": "Contradicts",
  "依赖": "Depends on",
  "引用": "References",
  "请选择另一张卡片完成连线": "Choose another card to complete the connection"
};

interface LocalizationContextValue {
  locale: AppLocale;
  setLocale(locale: AppLocale): void;
  t(source: string, values?: TranslationValues): string;
  runtime(source: string): string;
}

const defaultContext: LocalizationContextValue = {
  locale: "zh-CN",
  setLocale: () => undefined,
  t: (source, values) => interpolate(source, values),
  runtime: (source) => source
};

const LocalizationContext = createContext<LocalizationContextValue>(defaultContext);

export function LanguageProvider({ children, initialLocale }: { children: ReactNode; initialLocale?: AppLocale }) {
  const [locale, setLocaleState] = useState<AppLocale>(() => initialLocale ?? detectPreferredLocale());

  useLayoutEffect(() => {
    document.documentElement.lang = locale === "en" ? "en" : "zh-CN";
    document.title = locale === "en" ? "Personal Knowledge Agent" : "个人知识库 Agent";
  }, [locale]);

  const value = useMemo<LocalizationContextValue>(() => ({
    locale,
    setLocale(nextLocale) {
      setLocaleState(nextLocale);
      try {
        window.localStorage.setItem(LOCALE_STORAGE_KEY, nextLocale);
        const url = new URL(window.location.href);
        url.searchParams.set("lang", nextLocale === "en" ? "en" : "zh");
        window.history.replaceState(null, "", url);
      } catch {
        // Language selection still works for the current session.
      }
    },
    t(source, values) {
      return translateText(source, locale, values);
    },
    runtime(source) {
      return translateRuntimeText(source, locale);
    }
  }), [locale]);

  return <LocalizationContext.Provider value={value}>{children}</LocalizationContext.Provider>;
}

export function useLocalization(): LocalizationContextValue {
  return useContext(LocalizationContext);
}

export function detectPreferredLocale(options: {
  search?: string;
  storedLocale?: string | null;
  browserLanguages?: readonly string[];
} = {}): AppLocale {
  const search = options.search ?? (typeof window === "undefined" ? "" : window.location.search);
  const queryLocale = new URLSearchParams(search).get("lang")?.toLowerCase();
  if (queryLocale === "en" || queryLocale?.startsWith("en-")) return "en";
  if (queryLocale === "zh" || queryLocale?.startsWith("zh-")) return "zh-CN";

  let storedLocale = options.storedLocale;
  if (storedLocale === undefined && typeof window !== "undefined") {
    try {
      storedLocale = window.localStorage.getItem(LOCALE_STORAGE_KEY);
    } catch {
      storedLocale = null;
    }
  }
  if (storedLocale === "en" || storedLocale === "zh-CN") return storedLocale;

  const browserLanguages = options.browserLanguages ?? (typeof navigator === "undefined" ? [] : navigator.languages);
  return browserLanguages.some((language) => language.toLowerCase().startsWith("zh")) ? "zh-CN" : "en";
}

export function translateText(source: string, locale: AppLocale, values?: TranslationValues): string {
  const template = locale === "en" ? ENGLISH_MESSAGES[source] ?? source : source;
  return interpolate(template, values);
}

export function translateRuntimeText(source: string, locale: AppLocale): string {
  if (locale !== "en" || !source) return source;
  const exact = ENGLISH_MESSAGES[source];
  if (exact) return exact;

  const rules: Array<[RegExp, string]> = [
    [/^已只读载入 (.+)：(\d+) 篇 Markdown。$/, "Loaded $1 read only: $2 Markdown documents."],
    [/^已只读载入 (.+)：(\d+) 篇 Markdown，排除 (\d+) 个路径。$/, "Loaded $1 read only: $2 Markdown documents; $3 paths excluded."],
    [/^已载入 (.+)。$/, "Loaded $1."],
    [/^已连接 (.+)，读取 (\d+) 篇安全笔记，排除 (\d+) 个路径。$/, "Connected to $1: $2 safe notes loaded; $3 paths excluded."],
    [/^正在以只读方式打开 (.+)\.\.\.$/, "Opening $1 read only..."],
    [/^已以只读方式打开 (.+)。点击文件夹继续浏览，点击文件查看预览。$/, "Opened $1 read only. Open folders to browse and files to preview."],
    [/^无法打开 (.+)：(.+)$/, "Could not open $1: $2"],
    [/^只读预览：(.+) · (.+)。未修改原始文件。$/, "Read-only preview: $1 · $2. The original file was not changed."],
    [/^GitHub 公开库读取失败：(.+)$/, "Could not read the public GitHub repository: $1"],
    [/^公开仓库只读：允许 (\d+) \/ 排除 (\d+)$/, "Public repository read only: $1 allowed / $2 excluded"],
    [/^允许 (\d+) \/ 排除 (\d+)$/, "$1 allowed / $2 excluded"],
    [/^会话改动 (\d+) \/ 阻止 (\d+)$/, "Session changes $1 / blocked $2"],
    [/^当前模型：(.+)$/, "Current model: $1"],
    [/^上下文：约 (.+) \/ (.+) tokens；包含聊天、当前笔记、上传笔记和 vault 概览。$/, "Context: about $1 / $2 tokens, including chat, the current note, attached notes, and the knowledge base overview."],
    [/^(\d+) 个领域 · (\d+) 条跨域依据$/, "$1 domains · $2 cross-domain evidence links"],
    [/^(\d+) 篇文档 · 影响 (\d+) · 依据置信度 (\d+)%$/, "$1 documents · impact $2 · evidence confidence $3%"],
    [/^定义问题；被 (\d+) 篇文档依赖，并连接 (\d+) 个依据。$/, "Defines the problem; referenced by $1 documents and connected to $2 evidence links."],
    [/^提供证据；连接 (\d+) 个依据，并支撑当前领域。$/, "Provides evidence through $1 links and supports this domain."],
    [/^支撑决策；连接 (\d+) 个依据，并影响当前领域。$/, "Supports decisions through $1 links and influences this domain."],
    [/^沉淀成果；被 (\d+) 篇文档依赖，并形成可复用输出。$/, "Captures an output referenced by $1 documents and turns it into reusable knowledge."],
    [/^补充背景；提供 (\d+) 个连接与上下文。$/, "Adds background context through $1 connections."],
    [/^当前核心：(.+)$/, "Current core: $1"],
    [/^(.+) · 关系置信度 (\d+)%$/, "$1 · relationship confidence $2%"],
    [/^(\d+) 篇$/, "$1 docs"],
    [/^已框选 (\d+) 篇文档$/, "$1 documents selected"],
    [/^(\d+) 个已框选路径$/, "$1 selected paths"],
    [/^剩余 (\d+) 天 (\d+) 小时$/, "$1 days $2 hours remaining"],
    [/^剩余 (\d+) 小时$/, "$1 hours remaining"],
    [/^已到期，下一次刷新会清理$/, "Expired; it will be removed on the next refresh"],
    [/^(.+) 文件$/, "$1 file"]
  ];
  for (const [pattern, replacement] of rules) {
    if (pattern.test(source)) return source.replace(pattern, replacement);
  }
  return source;
}

function interpolate(template: string, values?: TranslationValues): string {
  if (!values) return template;
  return template.replace(/\{([A-Za-z0-9_]+)\}/g, (match, key: string) => String(values[key] ?? match));
}
